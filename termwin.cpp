...
Termwin::Termwin(int width, int height, char bg)
{
    ...
    size = width * height;
    for (int i = 0; i < size; i++) data += bg;
    topper.resize(size);
    std::fill(topper.begin(), topper.end(), "\033[0m");
}
    ...
    for (int i = 0; fmt[i] != '\0'; i++)
    {
        switch (fmt[i])
        {
            ...
            case 'l':
            {
                ...
                if (point.x > dimen.x || point.x < 0 || point.y > dimen.y || point.y < 0) { cx = px; cy = py; break; }
                px = cx;
                py = cy;
                cx = point.x;
                cy = point.y;
            ...
            case 'a': {
                ...
                topper[(cy * dimen.x) + cx] = std::string(attrib);
                break;
            }
        }
    }
...
void Termwin::draw()
{
    ...
    printf("\033[1;1H%s", outBuf.c_str());
    fflush(stdout);
}
